"""
Code from "https://github.com/GUDHI/TDA-tutorial"
"""
...
def WeightedRipsFiltration(X, F, p, dimension_max=2, filtration_max=np.inf):
    ...
    result_str = (
        "Weighted Rips Complex is of dimension "
        + repr(st.dimension())
        + " - "
...
def AlphaDTMFiltration(X, m, p, dimension_max=2, filtration_max=np.inf):
    ...
    result_str = (
        "Alpha Weighted Rips Complex is of dimension "
        + repr(st.dimension())
        + " - "
        + repr(st.num_simplices())
        + " simplices - "
...
def SampleOnNecklace(N_obs=100, N_out=0, is_plot=False):
    ...
    X2 = SampleOnSphere(N_obs, N_out=0) + [-1, 2 * 0.866, 0]
    X3 = SampleOnSphere(N_obs, N_out=0) + [-1, -2 * 0.866, 0]
